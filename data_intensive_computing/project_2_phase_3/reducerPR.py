...
current_dist, current_node, current_path, current_neighbours, sumOfPageRank = None, None, None, 0, 0
...
for line in sys.stdin:
    ...
    if len(line) == 3:
        # The line corresponds to a "child" pageRank update information.
    ...
    # For each node, collect all pageRank updates. If a new pageRank is smaller
    # than the current one, update the node pageRank and the path

    if current_node == node:
        # If one of the parents of the node provides a quicker path, choose it
        sumOfPageRank += current_dist
        current_dist, current_path = sumOfPageRank, path
        ...
        current_node, current_dist, current_path, current_neighbours = node, sumOfPageRank, path, neighbours
